...
export default function KnowYourValues ({ setFinalValues }) {
...
if(isFirstCharged) {
  ...
  return (
    ...
    <div className='card--new-value'>
        <p>{ currentValue && currentValue.name}</p>
        <button className='button-warning' onClick={getNewValue}>Muestra nuevo Valor</button>
        <button className="button-info"  onClick={() => showDescription(currentValue)}>Ver descripción</button>
      </div>
    <div className='values--container'>
      { initialValues.map(value => {
        return (
        <div key={value.name} className='card--value one'>
        <button className='button-warning' onClick={() => replaceValue(value)}>Sustituir</button>
